...
ds18b20_init(void)
{
    ...
    gpio_direction_output(temp_dev_3->dev_pin, 1);
    udelay(100);
    gpio_set_value(temp_dev_3->dev_pin, 0);
    udelay(485);
    gpio_set_value(temp_dev_3->dev_pin, 1);
    udelay(80);
    spin_lock_irqsave(&temp_dev_3->lock, flags);
    gpio_direction_input(temp_dev_3->dev_pin);
    ret = gpio_get_value(temp_dev_3->dev_pin);
    spin_unlock_irqrestore(&temp_dev_3->lock, flags);
    return ret;
} /*传感器初始化，发送复位脉冲*/
...
ds18b20_read8(void)
{
    ...
    {
        spin_lock_irqsave(&temp_dev_3->lock, flags);
        gpio_direction_output(temp_dev_3->dev_pin, 0);
        udelay(1);
        gpio_set_value(temp_dev_3->dev_pin, 1);

        gpio_direction_input(temp_dev_3->dev_pin);
        ret >>= 1;
        if (gpio_get_value(temp_dev_3->dev_pin))
        {
            ret |= 0x80;
        }
        spin_unlock_irqrestore(&temp_dev_3->lock, flags);
...
{
    filp->private_data = temp_dev_3; /* 设置私有数据 */
    if ((temp_dev_3 != NULL) && !temp_dev_3->device_open)
    {
        temp_dev_3->device_open = true;
...
{
    int result = 0;
    temp_dev_3 = kzalloc(sizeof(struct ds18b20_dev), GFP_KERNEL);
    if (!temp_dev_3)
    {
        result = -ENOMEM;
        goto err_free_mem;
    }
    result = alloc_chrdev_region(&temp_dev_3->devid, 0, DEV_CNT, DEV_NAME); /*申请设备号*/
    if (result < 0)
        goto alloc_err;

    cdev_init(&temp_dev_3->cdev, &ds18b20_fops); /*字符设备注册*/

    result = cdev_add(&temp_dev_3->cdev, temp_dev_3->devid, DEV_CNT); /*字符设备添加*/
    if (result < 0)
        goto cdevadd_err;

    temp_dev_3->ds18_class = class_create(THIS_MODULE, "ds18b20_3"); /*创建类*/

    temp_dev_3->ds18_device = device_create(temp_dev_3->ds18_class, NULL, temp_dev_3->devid, NULL, DEV_NAME); /*创建设备*/
    if (temp_dev_3->ds18_device == NULL)
        goto device_err;
    temp_dev_3->dev_node = of_find_node_by_path("/Temp3");
    if (temp_dev_3->dev_node == NULL)
    {

        goto fail2;
    }
    temp_dev_3->dev_pin = of_get_named_gpio(temp_dev_3->dev_node, "ds18b20_pin", 0);
    result = gpio_request(temp_dev_3->dev_pin, "ds18b20_pin");

    printk(KERN_EMERG "ds18b20_probe %d\r\n", result);
    spin_lock_init(&temp_dev_3->lock);
    printk("\n probe succsed \n");
    return 0;

fail2:
    cdev_del(&temp_dev_3->cdev);
...
{
    device_destroy(temp_dev_3->ds18_class, temp_dev_3->devid); /*摧毁设备，类+设备号*/
    class_destroy(temp_dev_3->ds18_class);
    cdev_del(&temp_dev_3->cdev);
    unregister_chrdev_region(temp_dev_3->devid, DEV_CNT);
    return 0;
}
